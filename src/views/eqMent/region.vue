...
<template>
    <div class="region">
        ...
        <Modal :visible="visible" :width="640" :title="myTitle" @isClose="isClose" @ok="isOK">
            ...
            >
                <a-input class="myInp" v-model:value="roleV.locationName" :maxlength="Ltxt.maxlength" placeholder="请输入用户名"/>
...
import { regionService, newS } from '../../http2/module/region'
    ...
    setup(){
        ...
        const gridOptions = reactive<VxeGridProps>({
            ...
            columns: [
                ...
                { field: 'locationName', title: '名称', showHeaderOverflow: true, treeNode: true },
        ...
        const getTab = async () => {
            ...
            // }
            // const res: any = await regionService.getSList(obj)
            // if(res.status == 200){
            //     gridOptions.data = []
            //     setTimeout(() => {
        ...
        const isOK = () => {
            formRef.value.validate().then(() => {
                const obj: newS = {
                    locationName: myData.roleV.locationName,
        ...
        // 提交数据
        const postVal = async (obj: newS, flag: boolean) => {
            if(flag){
                const res: any = await regionService.changeSData(obj)
                if(res.status == 200){
                    getTab()
                    isClose()
                }
            }else{
                const res: any = await regionService.newSData(obj)
                if(res.status == 200){
                    getTab()
                    isClose()
                }
        ...
        const someBtn = (flag: string, val: listType) => {
            switch(flag){
                case 'one' : 
                    ...
                    myData.visible = true;
                    myData.isShow = true;
                    myData.roleV.locationName = val.locationName
        ...
        const dlt = async (id: string) => {
            ...
            const res: any = await regionService.dltSItem(obj)
            if(res.status == 200){
                getTab()
            }
        }
