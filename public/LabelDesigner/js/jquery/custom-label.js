 ...
 (function ($) {
    ...
    CustomLabel.prototype = {
        ...
        initTopMenuLayout: function () {
            ...
            var topMenuTpl = [
                '<div class="cl-top-menu">',
                    ...
                    '<span><input style="width: 100px;" type="text" value= ' + that.opts.name +' id=3></span>',
                    '<label>规格尺寸</label>',
                    // '<span>' + that.opts.size[0] + 'mm*' + that.opts.size[1] + 'mm</span>',
                    '<span><input style="width: 60px;" type="text" value= ' + that.opts.size[0] +' id=1>mm* <input type="text" style="width: 60px;" value= ' + that.opts.size[1] +' id=2>mm*</span>',
            ...
            that.topMenu
                ...
                .on('blur', 'input', function (e) {
                    if (e.target.id === '1') {
                        inits({
                            name: that.opts.name,
                            size: [e.target.value, that.opts.size[1]]
                        }, that.getData().widgets)
                    } else if (e.target.id === '2') {
                        inits({
                            name: that.opts.name,
                            size: [that.opts.size[0], e.target.value]
                        }, that.getData().widgets)
                    } else if (e.target.id === '3') {
                        inits({
                            name: e.target.value,
                            size: [that.opts.size[0], that.opts.size[1]]
                        }, that.getData().widgets)
                    }
        ...
        onSave: function (data) {
            ...
            if (data.widgets[0].length === 0) return
            const oParam = {
              templatename: data.config.name,
              templatejson: JSON.stringify(data)
            }
            console.log(oParam)
            if (JSON.parse(sessionStorage.getItem('labelId')) !== '') {
                oParam.id = JSON.parse(sessionStorage.getItem('labelId'))

                // $.ajax({
                //     url: window.location.origin + '/api-manage/printTemplates',
                //     type: 'PUT',
                //     dataType: 'json',
